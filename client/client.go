...
import (
	...
	"github.com/asahipeak/terraform-provider-rizhiyi/client/utils"
)
...
func (c *Client) NewRequest(httpMethod, url string, body io.Reader) (*http.Request, error) {
	...
	if c.authToken != "" {
		...
		request.Header.Add("Authorization", "rizhiyi "+c.sessionKey)
	} else {
...
func (c *Client) Login() (e error) {
	loginURL := c.BuildRizhiyiURL(nil, "services", "auth", "login")
...
func NewDefaultRizhiyidClient() (*Client, error) {
	httpClient, err := NewRizhiyidHTTPClient(defaultTimeOut, true)
	if err != nil {
		return nil, err
	}
	c := &Client{auth: defaultAuth, host: defaultHost, httpClient: httpClient, userAgent: "Rizhiyi-simple-go-client"}
	return c, nil
}
...
func NewRizhiyidClient(sessionKey string, auth [2]string, host string, httpClient *http.Client) (*Client, error) {
	c, err := NewDefaultRizhiyidClient()
	if err != nil {
		return nil, err
	}
	c.auth = auth
